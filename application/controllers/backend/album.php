...
class Album extends CI_Controller{
    ...
    function simpan_album(){
        $config['upload_path'] = './assets/gambars/gallery/'; //path folder
        ...
        if(!empty($_FILES['filefoto']['name'])){
            if ($this->upload->do_upload('filefoto')){
                ...
                $config['source_image']='./assets/gambars/gallery/'.$gbr['file_name'];
                $config['create_thumb']= FALSE;
                $config['maintain_ratio']= FALSE;
                $config['quality']= '60%';
                $config['width']= 400;
                $config['height']= 350;
                $config['new_image']= './assets/gambars/gallery/'.$gbr['file_name'];
