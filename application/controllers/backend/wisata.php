...
class Wisata extends CI_Controller{
    ...
    function simpan_wisata(){
    	$config['upload_path'] = './assets/gambars/layanan/'; //path folder
	    ...
	    if(!empty($_FILES['filefoto']['name'])){
	        if ($this->upload->do_upload('filefoto')){
	            ...
	            $config['source_image']='./assets/gambars/layanan/'.$gbr['file_name'];
	            $config['create_thumb']= FALSE;
	            $config['maintain_ratio']= FALSE;
	            $config['quality']= '60%';
	            $config['width']= 756;
	            $config['height']= 434;
	            $config['new_image']= './assets/gambars/layanan/'.$gbr['file_name'];
